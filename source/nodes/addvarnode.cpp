...
AddVarNode::AddVarNode(std::shared_ptr<Passible> passible, std::shared_ptr<Token> token, std::string name, std::shared_ptr<Node> value)
    : Node(passible, token)
{
    this->name = name;
    this->value = value;
...
std::shared_ptr<Variable> AddVarNode::execute(std::shared_ptr<Scope> scope)
{
    ...
    else
    {
        ...
        if (var != nullptr)
        {
            ...
            {
                passible->errors->add(std::make_shared<Error>(RUNTIME_ERROR, "Variable not declared", token));
            }
        }
    }
    return null;
