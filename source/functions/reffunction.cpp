...
namespace HuwInterpreter {
    namespace Functions {
        RefFunction::RefFunction(std::shared_ptr<Passable> passable)
            : Function(passable)
        {
            setName("ref");
        }

        std::shared_ptr<Variable> RefFunction::execute(std::shared_ptr<Tokens::Token> token, std::shared_ptr<Scope> globalScope,
