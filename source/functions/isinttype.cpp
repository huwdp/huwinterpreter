...
namespace HuwInterpreter {
    namespace Functions {
        IsIntType::IsIntType(std::shared_ptr<HuwInterpreter::Passable> passable)
            : Function(passable)
        {
            setName("isIntType");
        }

        std::shared_ptr<Variable> IsIntType::execute(std::shared_ptr<Tokens::Token> token, std::shared_ptr<Variables::Scope> globalScope,
