...
namespace HuwInterpreter {
    namespace Functions {
        ToNumber::ToNumber(std::shared_ptr<HuwInterpreter::Passable> passable)
            : Function(passable)
        {
            setName("toInt");
        }

        std::shared_ptr<Variable> ToNumber::execute(std::shared_ptr<Tokens::Token> token, std::shared_ptr<Variables::Scope> globalScope,
