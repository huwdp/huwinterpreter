...
namespace HuwInterpreter {
    namespace Functions {
        UnrefFunction::UnrefFunction(std::shared_ptr<Passable> passable)
            : Function(passable)
        {
            setName("unRef");
        }

        std::shared_ptr<Variable> UnrefFunction::execute(std::shared_ptr<Tokens::Token> token, std::shared_ptr<Scope> globalScope,
