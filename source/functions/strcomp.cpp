...
namespace HuwInterpreter {
    namespace Functions {
        StrComp::StrComp(std::shared_ptr<HuwInterpreter::Passable> passable)
            : Function(passable)
        {
            setName("strCmp");
        }

        std::shared_ptr<Variable> StrComp::execute(std::shared_ptr<Tokens::Token> token, std::shared_ptr<Variables::Scope> globalScope,
                                               ...
                                               std::vector<std::shared_ptr<Nodes::Node>> arguments)
        {
            if (arguments.size() == 2)
            {
                ...
                return std::make_shared<DoubleVariable>(passable, diff);
            }
