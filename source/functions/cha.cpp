...
namespace HuwInterpreter {
    namespace Functions {
        Cha::Cha(std::shared_ptr<HuwInterpreter::Passable> passable)
            : Function(passable)
        {
            setName("cha");
        }

        std::shared_ptr<Variable> Cha::execute(std::shared_ptr<Tokens::Token> token, std::shared_ptr<Variables::Scope> globalScope,
                                           ...
                                           std::vector<std::shared_ptr<Nodes::Node>> arguments)
        {
            if (arguments.size() == 1)
            {
                ...
                try
                {
                    ...
                    char c = ascii;
                    output.append(1,c);
                    return std::make_shared<StringVariable>(passable, "",output);
                }
