...
namespace HuwInterpreter {
    namespace Functions {
        ToLower::ToLower(std::shared_ptr<HuwInterpreter::Passable> passable)
            : Function(passable)
        {
            setName("toLower");
        }

        std::shared_ptr<Variable> ToLower::execute(std::shared_ptr<Tokens::Token> token, std::shared_ptr<Variables::Scope> globalScope,
