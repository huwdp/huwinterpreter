...
namespace HuwInterpreter {
    namespace Functions {
        Count::Count(std::shared_ptr<HuwInterpreter::Passable> passable)
            : Function(passable)
        {
            setName("count");
        }

        std::shared_ptr<Variable> Count::execute(std::shared_ptr<Tokens::Token> token, std::shared_ptr<Variables::Scope> globalScope,
