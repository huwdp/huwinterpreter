                                     ...
                                     std::vector<std::shared_ptr<Node>> variables)
{
    ...
    if (variables.size() == 2)
    {
        ...
        if (var1 != nullptr && var2 != nullptr)
        {
            ...
            {
                passable->errors->add(std::make_shared<Error>(FUNCTION_ERROR, "Invalid argument in Atan2", token));
            }
            catch (const std::out_of_range ex)
            {
                passable->errors->add(std::make_shared<Error>(FUNCTION_ERROR, "Out of range in Atan2", token));
            }
    ...
    {
        passable->errors->add(std::make_shared<Error>(FUNCTION_ERROR, "Abs function requires one argument", token));
    }
    return returnNode;
}
