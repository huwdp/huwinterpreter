...
namespace HuwInterpreter {
    namespace Functions {
        Asc::Asc(std::shared_ptr<HuwInterpreter::Passable> passable)
            : Function(passable)
        {
            setName("asc");
        }

        std::shared_ptr<Variable> Asc::execute(std::shared_ptr<Tokens::Token> token, std::shared_ptr<Variables::Scope> globalScope,
                                           ...
                                           std::vector<std::shared_ptr<Nodes::Node>> arguments)
        {
            if (arguments.size() == 1)
            {
                ...
                if (temp.size() == 1)
                {
                    ...
                    return std::make_shared<DoubleVariable>(passable, ascii);
                }
		return nullVariable;
            }
