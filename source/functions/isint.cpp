...
namespace HuwInterpreter {
    namespace Functions {
        IsInt::IsInt(std::shared_ptr<HuwInterpreter::Passable> passable)
            : Function(passable)
        {
            setName("isInt");
        }

        std::shared_ptr<Variable> IsInt::execute(std::shared_ptr<Tokens::Token> token, std::shared_ptr<Variables::Scope> globalScope,
                                      ...
                                      std::vector<std::shared_ptr<Nodes::Node>> arguments)
        {
            if (arguments.size() == 1)
            {
                ...
                return std::make_shared<DoubleVariable>(passable, Helpers::TypeDetector::isInteger(var->toString()));
            }
