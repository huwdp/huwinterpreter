...
namespace HuwInterpreter {
    namespace Functions {
        IsDouble::IsDouble(std::shared_ptr<HuwInterpreter::Passable> passable)
            : Function(passable)
        {
            setName("isDouble");
        }

        std::shared_ptr<Variable> IsDouble::execute(std::shared_ptr<Tokens::Token> token, std::shared_ptr<Variables::Scope> globalScope,
                                                ...
                                                std::vector<std::shared_ptr<Nodes::Node>> arguments)
        {
            if (arguments.size() == 1)
            {
                ...
                if (!Helpers::TypeDetector::isInteger(var->toString()) && Helpers::TypeDetector::isNumeric(var->toString()))
                {
